#include "numberToHangul/numberToHangul.hpp"
...
TEST_CASE("numberToHangul", "[numberToHangul]")
{
    ...
    SECTION("유효하지 않은 숫자에 대한 오류 처리")
    {
        std::string ERROR_MESSAGE = "유효한 0 이상의 정수를 입력해주세요.";
        REQUIRE_THROWS(numberToHangul(-1), ERROR_MESSAGE);
        REQUIRE_THROWS(numberToHangul(-12345), ERROR_MESSAGE);
    }
}
} // namespace NumberToHangul
} // namespace EsHangul
